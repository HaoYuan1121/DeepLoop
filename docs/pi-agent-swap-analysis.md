...
**动机（为什么值得做）**：实践中 pi-agent 的 loop engineering 完成度高、效率好，能以较小开销基本成功完成大部分任务；而 DSH 的会话日志折叠请求头、增量追加事件，KV-cache 命中率极高。用 PI 的 loop 驱动 DSH 缓存友好的会话层，有望显著节省 token——loop 的请求更少更精简，harness 又最大化缓存复用。这一节省目前仍是假设、未经实测，实际体验有待真实用户验证。
