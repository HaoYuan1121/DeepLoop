...
agent（智能体）的唯一具体实现插件和循环驱动器——在 deepseek-pi 中，由 **PI agent loop**（"loop engineering"，循环工程）驱动，该引擎以 `@deepseek-pi/pi-agent-core` 形式 vendored 到仓库。`PiLoopAgent` 适配器满足 `Agent` 接口，并通过 PI 的 `runAgentLoop` 驱动会话、轮次和步骤的生命周期，而 harness 的 LLM 接缝、工具流水线和会话日志仍然保持权威。
...
具体 `PiLoopAgent`（驱动引擎为 vendored 的 PI `runAgentLoop`，见 [vendor/README.md](../../../vendor/README.md)）、其 inbox 与运行控制均为包内部实现。包根只导出插件／服务／配置约定，包导出映射不提供 `./src/*` 逃逸路径；生命周期拥有方通过 `ctx.agents` 创建 agent，而不是点名、构造或启动驱动器内部组件。一个准备完成的会话只能由一个具体驱动器认领；所有可观测行为都通过会话事件和 `agent/*` 事件分类体系发生。
