/**
 * Concrete agent-loop plugin: creates scoped PiLoopAgents (driven by the
 * vendored PI agent loop), publishes them through the agent/session
 * registries, and owns their ordered teardown.
 *
...
import { PiLoopAgent } from './pi-agent.ts'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.config = {
      ...
      // Read through on every scheduler decision: the PI loop reads the cap
      // per run, so a committed change caps the next run without disturbing
      // the one in flight. The PI loop currently executes tools sequentially;
  ...
  private prepare(ownerCtx: Context, id: SessionId, options: AgentOptions, session: Session, callerSignal?: AbortSignal): PreparedAgent {
    ...
    let machine: PiLoopAgent | undefined
    ...
    try {
      const agent = machine = new PiLoopAgent(loopCtx, id, options, session)
